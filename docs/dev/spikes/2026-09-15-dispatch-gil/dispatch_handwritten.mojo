...
struct Saw(Copyable, Movable, Writable):
    var phase: Float32
    var inc: Float32
...
struct Gain(Copyable, Movable, Writable):
    var g: Float32
...
@always_inline
def node_process(mut n: Node, src: FPtr, dst: FPtr, k: Int):
    if n.isa[OnePole]():
        n[OnePole].process(src, dst, k)
    elif n.isa[Gain]():
        n[Gain].process(src, dst, k)
    else:
        n[Saw].process(src, dst, k)
...
@always_inline
def node_tick(mut n: Node, x: Float32) -> Float32:
    if n.isa[OnePole]():
        return n[OnePole].tick(x)
    elif n.isa[Gain]():
        return n[Gain].tick(x)
    else:
        return n[Saw].tick(x)
